...
import { RefreshCw, Download, ArrowLeft, Trash2 } from 'lucide-react';
    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 py-3.5 px-4 sm:px-8 shadow-sm w-full">
      <div className="w-full max-w-[1920px] mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          {/* Clear / Reset All Orders Button */}
          {onClearOrders && (
            <button
              onClick={onClearOrders}
              className="bg-slate-100 hover:bg-rose-50 text-rose-600 hover:text-rose-700 border border-rose-200 font-bold text-xs px-3 py-2 rounded-xl transition-all flex items-center gap-1.5 cursor-pointer"
              title="Clear all test orders & reset dashboard"
            >
